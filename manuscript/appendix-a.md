...
Во овој дел ќе бидат покажани неколку примери на имплементации на техники кои се користат во Z3. Поконкретно, имплементацијата од А.2 соодветствува на клучниот збор `calc`, a имплементацијата од А.3 и А.4 на автоматско докажување на теореми.

Lisp е програмски јазик имплементиран во 1958 година од информатичарот Џон Меккарти (1927 - 2011) [13]. Синтаксите се тоа што го прават Lisp таканаречен _симболичен_ јазик - можноста за конструкција и манипулација со симболи. Поради таа намена, истиот ќе биде искористен за пример имплементација на системи за математички докази.

> _Дефиниција А.1_: **Апстрактно синтактичко дрво** е дрво во кое се претставуваат команди од изворен код напишан во програмски јазик.

Кога се пишуваат програми во програмски јазик, постои среден чекор што го анализира изворниот код на истите и произведува апстрактно синтактичко дрво. Потоа, ова дрво се евалуира/пресметува.

![Апстрактно синтактичко дрво\label{ast}](images/ast.png)

На пример, слика \ref{ast} претставува апстрактно синтактичко дрво за следниот код:
...
Во Lisp нема ограничување на посебна синтакса како што имаат други програмски јазици. Пишаниот код ќе биде всушност апстрактното синтактичко дрво. Оттука, Lisp се залага на минималистички дизајн со тоа што не постои overhead од дополнителни специјални синтакси.

Во дефиниција 5.1 беа дефинирани формални системи и формален јазик, каде синтаксата има посебно значење во Lisp. S-expressions ја формираат синтаксата на Lisp.

> _Дефиниција А.2_: Синтактичките елементи во Lisp се симболични изрази или **S-expressions** [13]. Тие имаат вредност кој е листа или симбол.
...
На пример, со `(list 1 2 3)` се произведува низата $(1, 2, 3)$ и се добива `'(1 2 3)` како резултат. Во овој случај, се искористи `list`, што е вградена функција која прифаќа било кој број на параметри и како резултат, враќа листа генерирана од нив.

Во Lisp се користат загради за означување на повик за функција (евалуација). Во принцип, кодот `(f a_1 a_2 ... a_n)` прави евалуација на `f` со `n` параметри. На пример, за функцијата $f(x) = x + 1$, една евалуација е $f(1)$ каде како повратна вредност се добива 2, а се пишува `(f 2)`. Сепак, како резултат се добива `'(1 2 3)`. Во спротивно, ако се вратеше `(1 2 3)` тогаш тоа немаше да има смисла, бидејќи, како што беше дискутирано, оваа нотација ќе се обиде да направи повик на функцијата `1` со аргументи `2` и `3`. Наместо тоа, се добива `'(1 2 3)` односно quoted листа - листа во "наводници". Ова е налик дијагонализацијата во дефиниција 5.29.

Покрај `list`, постојат и други битни вградени функции [21]:

- `cons` - креира пар од две вредности. На пример, `(cons 1 2)` е пар каде првата вредност е 1 а втората 2
...
- `map` - применува функција врз секој елемент од листа. На пример, `(map f '(1 2 3))` ќе врати `(list (f 1) (f 2) (f 3))`
...
- `struct` - креира структура - комплексен податок содржан од други комплексни/примитивни податоци. На пример, `(struct kniga (ime avtor))` ќе креира неколку функции:
  - `kniga x y` - за создавање на нова книга со име `x` и автор `y`
  - `kniga-ime k` - за преземање на име од дадена книга
  - `kniga-avtor k` - за преземање на автор од дадена книга
...
Во овој случај, `dodadi-1` е функција што додава 1 на некој број. На пример, `(dodadi-1 1)` ќе врати 2. Оваа функција може да се дефинира на по-елегантен начин, каде двете дефиниции се еквивалентни:

```{language=racket}
(define (dodadi-1 x) (+ x 1))
```

На овој начин се врши евалуација, а исто така како може да се конструираат овие S-expressions - листи или симболи. Ова е сосема доволно за да може да се програмира во овие системи - оттука минималистичкиот дизајн.

## А.1. Пример: Само-репродуцирачки програми

Со самата моќ што ја има Lisp за обработка на симболи, може на едноставен начин да се напише програма што како резултат ќе се врати кодот на самата програма[^apa1] [1]. Тоа се постигнува со користење на дијагонализација, односно `quote` [8]. Следниот код се надоврзува на примерите од дефиниција 5.29:
...
Во дефиницијата на `q` се искористи `(d identity x)`, односно дијагонализација на `x`. Со замена може да се види дека ова е едноставно `(list 'quote x)`. Следно, ќе биде покажан изразот `(list x (list 'quote x))` кој враќа листа што содржи два елементи: `x` и неговата дијагонализација.
...
Во второто извршување се врати истиот код кој се искористи, односно самата програма. Ова е причината зошто беше одбран изразот `(list x (list 'quote x))` - евалуацијата на истиот треба да го врати целиот израз.

Конечниот код ќе биде примена на `quote`-ирана верзија од `lambda`-та врз истата:
...
1. `succ` се дефинира како функција што прима еден параметар `x`, а враќа листа каде пред `x` се наоѓа симболот `s`. На пример, `(succ zero)` ќе се евалуира во `'(s z)`
...
На крај, дадена теорема е валидна ако двете страни од равенството се валидни.
...
Функцијата `prove-theorem` дефинира чекори каде во секој чекор се трансформира зададената теорема (онаа што треба да се докаже) со тоа што се користи правило на заклучување. На крај, се враќа последната трансформација `step2`. Во овој случај, `(valid-theorem? (prove-theorem))` ќе врати вистина.

На овој начин се покажа минимален систем за конструирање докази во кој може да се конструираат равенки и да се прават замени во истите, сличен на командата `calc` во Dafny [4].

## А.3. Пример: Автоматски докажувач на теореми

Во 5.1 беше разгледано за формални системи. Во овој пример, ќе биде имплементиран автоматски докажувач на теореми за даден формален систем [6]. Треба да се специфицира кои се аксиомите и правилата на заклучување а потоа програмата ќе ги проба сите можни комбинации и ќе врати дали се постига конечниот (бараниот) резултат.
...
За да се најде доказ за даден `theorem-prover`, се пребарува низ сите можни резултати и се проверува дали заклучокот се содржи. Ако се содржи, само се враќа. Во спротивно, рекурзивно се градат нови теореми на начин каде сите правила се применуваат едно по друго.
...
Функцијата за наоѓање на докази во вакви бесконечни дрва е:
...
Како резултат се добива `(theorem '("MI") (list (rule "One" #) (rule "Two" #)) "MIUIU")`, што вели дека за почетна теорема `MI` се применува правило `One`, па `Two` за да се стигне до бараниот заклучок.

## А.4. Пример: Програмирање со ограничувања

Во овој начин на програмирање, проблемот се претставува декларативно наместо да се пишува алгоритам за решавање на истиот. Слично беше постигнато и со дефинирање на `mu-rules` претходно, каде проблемот беше моделиран преку листа на правила, но таму се користеа правила за да изменат теорема, додека во програмирање со ограничувања се користат правила за проверување на релации.

Овој начин на програмирање е исто така познат како Constraint Programming, а математички се претставува преку тројката $(X, D, C)$ каде:

- $X = \{ x_1, x_2, \ldots, x_n \}$ е множество на променливи
- $D = \{ d_1, d_2, \ldots, d_n \}$ е множество на домени (каде на секое $x_i$ му се придружува $d_i$)
- $C = \{ c_1, c_2, \ldots, c_m \}$ е множество на ограничувања (constraints), каде секое ограничување $c_k$ е пар $(S, R)$ за кој важи:
  - $S = (x_{i_1}, \ldots, x_{i_k})$ се променливи во $c_k$
  - $R \subseteq d_{i_1} \times \ldots \times d_{i_k}$ се $k$-торки кои ја задоволуваат релацијата $c_k$
...
Функцијата за наоѓање на решението се претставува на следен начин:
...
На овој начин се покажа минимален систем за конструирање автоматски докази, во природа сличен на оној имплементиран во Dafny [18].

[^apa1]: Овој концепт се нарекува Quine а се однесува на само-репродуцирачки програми, односно мета програми - програми кои се враќаат сами себе по извршување.
